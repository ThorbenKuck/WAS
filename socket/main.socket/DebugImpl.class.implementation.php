...
class DebugImpl implements Debug {
	...
	public function __construct(Settings $settings, LowLevel $user) {
	...
	function log ($string, $log_format = "old", $custom_name = null) {
		...
		switch ( $log_format ) {
			...
			case "modern":
				...
				break;
			case"console":
				if ($this->user->is_logdin ()) {
					$start = $this->user->get_username () . "@" . $_SERVER ["REMOTE_ADDR"] . " [" . date ('H') . ":" . date ('i') . ":" . date ('s') . "] $ ";
				} else {
					$start = $this->user->get_username () . "@" . $_SERVER ["REMOTE_ADDR"] . " [" . date ('H') . ":" . date ('i') . ":" . date ('s') . "] $ ";
				}
				$end = PHP_EOL;
		...
		if ($custom_name !== null) $current_log = $this->settings->accessCertain('root_path') . "logs/" . $custom_name;
		else $current_log = $this->find_current_log ();

		$handle = fopen ($current_log, "a") or $this->debug_input ("(Error)Could not open the current log ... See further stacktrace for information");
	...
	function debug_mod_enabled () : bool {
		return $this->settings->accessCertain('debug_mode');
	}
	 ...
	 * @version 1.0
	 *
	 * @return mixed Returns NULL if the config is not loaded and false if it cant create the new log. If everything goes well, it returns the path to the current log
	 */
	function find_current_log () : mixed {
		...
		$fallback = false;
		$path_to_logs = $this->settings->accessCertain('root_path') . "logs/";
